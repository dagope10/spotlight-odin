...
void main() {
  ...
  float rect_alpha = 1.0 - smoothstep(0.0, 1.0, dist);
  if (uIsText) {
    float text_alpha = texture(atlasText, FragUV).r;
    fragColor = vec4(uColor.rgb, uColor.a * text_alpha);
  } else {
    fragColor = vec4(uColor.rgb, uColor.a * rect_alpha);
  }
}
